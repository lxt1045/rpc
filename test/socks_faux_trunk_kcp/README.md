...
1. **发送窗口 `kcp_sndwnd` 是限速链路上最关键的一项，必须按 BDP 设**：
   ...
   | --- | --- | --- | --- | --- | --- |
   | 1024（库默认，≈8×BDP） | **3.2~3.8x** | 68% | 62~65% | 2.3 MB/s | 101% |
...
2. **但绝不能写死成很小的值**——真机实测：把发送/接收窗口都设成 128 段后，这条
   RTT 较大的链路被自己饿死，出口只占 **7Mbps**、下载掉到 **300kB/s**。原因是窗口
   本身成了唯一限速手段，一旦小于 BDP（速率×RTT）就填不满链路。所以先 `ping` 出
   真实 RTT：30Mbps+RTT40ms≈128 段，RTT150ms≈480 段，RTT200ms≈640 段。

3. **接收窗口 `kcp_rcvwnd` 只做缓冲，不要跟着一起调小**（库默认 1024，通常不用改）。
   调小后应用（下游 TCP/浏览器背压）稍有停顿就会关闭通告窗口，把对端饿死——上面
   的真机故障里它有一半责任。

4. **`kcp_auto_wnd`（实验性）**：发送窗口按"线上放大率"自动伸缩（`trunk_kcp/autownd.go`、
   `SetAutoWindow`）。40ms 无损限速链路上实测能稳定收敛到"吃满链路、放大≈1.2"；
   长 RTT/有损链路上仍在调优（会振荡），**生产建议先用固定 `kcp_sndwnd` 按 BDP 设**。
   打开后每 3s 会有一行 debug 自诊断日志，直接给出调窗依据：

   ```
   trunk_kcp: 调窗 snd=128 rcv=1024(应用积压 2048 段) 线上 3000 KB/s 已确认 2950 KB/s
              放大 1.02(ewma 1.04) 重传 3.1% RTT srtt=42ms min=40ms
   ```

   判读：`放大` 接近 1 且链路被吃满 = 窗口合适；`放大` > 1.5 = 窗口超过 BDP+队列
   （该调小）；线上速率远低于链路且 `放大`≈1 = 窗口太小（该调大）。

5. **`kcp_nc: 0`（打开 kcp-go 自带拥塞控制）不是解**：它的 cwnd 爬升太慢，实测只用到
   链路 5~8%，吞吐反降到 0.2 MB/s。限速链路上正确组合是"关拥塞控制（默认）+ 按 BDP
   设窗口"。
