...
2. **先分清"限速"还是"有损"——两者调参方向相反**（这一步比选具体数值重要）：
   ...
   这类链路吞吐 ≈ `在途 × (1-丢包率) / 修复时延`，所以**窗口翻倍吞吐翻倍**，并且要把
   修复时延压下来（`resend=2` + `nodelay=1`）。实测（45% 丢包、RTT 54ms，
   `TestLossyPathTuning`）：snd32 0.22MB/s → snd96 0.44 → snd96+resend2 0.46 →
   snd96+resend2+minRTO30 0.59 → **snd192+resend2+minRTO30 1.00MB/s**。
   推荐：`kcp_sndwnd: 192`、`kcp_resend: 2`、`kcp_nodelay: 1`、`kcp_nc: 1`。
...
4. **`kcp_auto_wnd`（实验性，真机实测失败，生产勿用）**：发送窗口按"线上放大率"自动
   伸缩（`trunk_kcp/autownd.go`、`SetAutoWindow`）。40ms 无损内存链路上能收敛，但真机
   （RTT 54ms）上它把窗口从 64 缩到下限 **32 段**，伴随重传、srtt 被抬到 144ms，吞吐
   从 1.05MB/s 掉到 **150kB/s**（同时把 yml 里的 `kcp_sndwnd: 192` 覆盖掉，日志里只显示
   `snd=32` 很难看出原因）。**请用固定 `kcp_sndwnd`**；确需实验请配合启动日志确认实际
   生效值。注：事后看它收敛到的 32 段本身与本文的 BDP 结论一致（当次日志里的 42% 是
   **累计**重传率，混入了大窗口阶段的数据）；失败点在于"自动调窗会把配置覆盖掉"且
   收敛过程本身制造了伪重传。
...
7. **`kcp_resend` 不是这一层的旋钮**：窗口按 BDP 设对之后，`resend=2` 与 `resend=32`
   实测重传率几乎一样（17.9% vs 17.0%），因为残余重传不是"真丢包等 RTO"，而是下一条
   的乱序伪重传。先别在这上面花时间。
