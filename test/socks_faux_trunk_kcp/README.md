...
1. **发送窗口 `kcp_sndwnd` 是限速链路上最关键的一项，必须按 BDP 设**：

   ```
   kcp_sndwnd ≈ 链路速率(B/s) × RTT(s) / (kcp_mtu-24)      # 再留 20%~50% 余量
   ```

   它是"在途数据上限"。库默认 1024 段在 30Mbps 出口上相当于 8×BDP：每一轮都把出口
   整形/限速队列灌爆 → 丢包 → 重传 → 带宽被重传吃掉。实测
   （`trunk_kcp/ratelimit_test.go`，24Mbps、RTT 40ms、排队上限 50ms）：

   | 发送窗口（段） | 线上放大 | 重传段占比 | 瓶颈丢包 | 有效吞吐 | 链路利用 |
   | --- | --- | --- | --- | --- | --- |
   | 1024（库默认，≈8×BDP） | **3.2~3.8x** | 68~72% | 62~65% | 2.3 MB/s | 101% |
   | 256 | 1.38~1.65x | 25~34% | 12~19% | 2.3 MB/s | 94~101% |
   | **128（≈BDP）** | **1.17~1.27x** | 11~19% | ~0% | **2.4~2.6 MB/s** | **98~101%** |
   | 32（<BDP） | 1.05x | 2~6% | 0% | 1.0 MB/s | 32~36% |

   真机三组对照实验（43.155.182.37，RTT 54ms，服务端 30Mbps 网卡）验证了同一规律——
   **在途超过 BDP 后，代价直接落在网卡上（网卡跑 5~6Mbps、下载只有 2.5Mbps）**：

   | 实验 | 配置 | 下载 | 服务端线上 | 线上放大 | 区间重传 | 队列丢包 |
   | --- | --- | --- | --- | --- | --- | --- |
   | A | `snd=96 nc=1 resend=32` | 250kB/s | 5~6.6Mbps | **1.8~2.3x** | **46~53%** | 高（重传大多又被丢） |
   | B | `snd=96 nc=0 resend=32` | 180kB/s | 3.5Mbps | 1.4~2.1x | 40~50% | 高 |
   | C | `snd=96 nc=0 resend=2` | 50kB/s | 560kbps | 0.97x | 5% | ~0（但吞吐塌了） |

   A 的机理已用 `trunk_kcp/ratelimit_test.go` 的
   `TestWindowOvershootSpuriousRTO` 复现：`snd=96`（≈3×BDP）时瓶颈丢包 **33%**、
   重传 **43.7%**、放大 **1.82x**；换成 `snd=32`（≈BDP）后丢包 **0%**、放大降到
   **1.21~1.24x**、链路利用仍是 **100%**。

2. **先分清"限速"还是"有损"——两者调参方向相反**（这一步比选具体数值重要）：

   | 日志特征 | 链路类型 | 窗口 |
   | --- | --- | --- |
   | `放大` 随窗口增大而升高、吞吐在某窗口后**下降** | 限速（整形/QoS） | 按 BDP 设，宁小勿大 |
   | `重传 ≈ 固定丢包率`（不随窗口变）、吞吐随窗口**单调上升** | **有损（链路丢包）** | **越大越好**，BDP 公式不适用 |

   真机 2026-09-23 14:55 那次属于**有损**：snd=32 时下载 60kB/s、服务端网卡 1.6Mbps、
   重传 58~63% 且几乎全是 `提前`（`快 0`）、客户端 `收段重复≈0.5%`（重传的原始包真丢了）。
   这类链路吞吐 ≈ `在途 × (1-丢包率) / 修复时延`，所以**窗口翻倍吞吐翻倍**。实测
   （45% 丢包、RTT 54ms，`TestLossyPathTuning`）：snd32 0.22MB/s → snd96 0.44 →
   **snd192 1.00MB/s**。同样的扫描里 `resend`（2 vs 32）与 `minRTO`（100ms vs 30ms）
   的差异远小于窗口、且重复跑抖动很大，因此**不作为配置项**（见下面第 7 条）。
   推荐：`kcp_sndwnd: 192`（越大越好）、`kcp_nodelay: 1`、`kcp_nc: 1`、`kcp_rcvwnd: 1024`。

   反之，若日志显示 `放大` 随窗口升高、吞吐有峰值（限速链路），才用 BDP 公式：
   30Mbps+RTT40ms≈128 段、RTT150ms≈480 段、RTT200ms≈640 段，`ping` 出真实 RTT 再算。

3. **接收窗口 `kcp_rcvwnd` 只做缓冲，不要跟着一起调小**（库默认 1024，通常不用改）。
   调小后应用（下游 TCP/浏览器背压）稍有停顿就会关闭通告窗口，把对端饿死——上面
   的真机故障里它有一半责任。

4. **自动调窗（`kcp_auto_wnd` / `SetAutoWindow`）：已实测失败并删除**。它按"线上放大率"
   伸缩发送窗口：40ms 无损内存链路上能收敛，但真机（RTT 54ms）上把窗口从 64 缩到下限
   **32 段**，吞吐从 1.05MB/s 掉到 **150kB/s**，而且会覆盖 yml 里的 `kcp_sndwnd: 192`
   （日志里只显示 `snd=32`，很难看出原因）。教训是**窗口必须由使用者按链路显式设定**，
   不要交给控制器猜；相关代码与配置项已移除，只保留 `SetWindowSize` 与自诊断日志。

5. **`kcp_nc: 0`（打开 kcp-go 自带拥塞控制）不是解**：kcp-go v5.4.20 的 cwnd 只有两档——
   任何 RTO 直接 `cwnd=1`；任何"提前重传"（`fastack>0` 且窗口打满，此时每轮都会命中）
   走 rate-halving `cwnd=inflight/2+resend`。于是 `resend=2` 时 cwnd 被钉在 **4~6 段**
   （实测在途 6 段、只用到 36% 链路、goodput 0.17 对 nc=1 的 0.41 MB/s），正是实验 C
   "链路很干净但只有 50kB/s"的原因。正确组合是"关拥塞控制（默认）+ 按 BDP 设窗口"。

6. **长 RTT 链路建议 `kcp_nodelay: 0`**（minRTO 100ms）：真机等效 RTT 较大时，
   `nodelay=1` 的 30ms minRTO 会把"还在路上的包"当成丢包重传（伪重传），实测长 RTT
   场景下放大 2.07x/重传 37%；用 100ms minRTO 可显著减少。物理层是不可靠的 faux_tcp，
   短 RTT 链路用默认快速模式即可。

7. **`kcp_resend` / `kcp_interval` 已删除（实测无效）**：窗口设对之后，`resend=2` 与
   `resend=32` 的重传率几乎一样（17.9% vs 17.0%），因为残余重传不是"真丢包等 RTO"，
   而是多连接乱序造成的伪重传（见下一条）；45% 随机丢包下 `resend`/`minRTO` 的收益
   也远小于窗口、重复跑抖动很大。所以配置项收敛为
   `kcp_mtu / kcp_sndwnd / kcp_rcvwnd / kcp_nodelay / kcp_nc` 五项。

8. **多物理连接会因乱序白白重传（已知损耗，约 16%）**：一条 trunk 的 KCP 段由多个
   `sendLoop` 竞争同一个 `sendChan` 发出、接收端多个 `recvLoop` 竞争 `recvChan`，SN
   顺序被打乱；kcp-go 的 early retransmit 会把乱序当丢包，重传**已经到达**的段，这些
   重复段真的穿过链路、直接吃掉带宽。实测（512KB/s、RTT 54ms、瓶颈零丢包、`snd=32`）：
   1 条物理连接 → 乱序 **0%**、重传 **0%**、goodput **0.49MB/s**；4 条 → 乱序 **27.7%**、
   重传 **16.3%**、goodput **0.41MB/s**。
   修法（尚未实现）：① 接收侧喂 `kcp.Input` 前按 SN 小缓冲排序；② 发送侧改成"每轮
   flush 粘在同一条物理连接、轮转切换"，而不是多 goroutine 抢同一个 channel。

   **注意别把这条当成有损链路的解释**：接收侧 `乱序` 计数是"SN 小于已见最大 SN 的新段"，
   它同时包含真乱序与"原始包丢了、重传后才到"的段。所以在丢包链路上
   `乱序 ≈ 重传` 是正常的（2026-09-23 14:55 那次：`乱序 42~57%`、`重复 0.5%`、`重传 58~63%`，
   这是**丢包**的表现，不是多连接乱序）。要确认多连接乱序，必须在**零丢包**链路上比
   不同连接数——上面那组数字就是这么测的。

9. **自诊断日志**（每 3s 一行 debug，固定窗口也打）读法：
   ...
   - `线上`/`收线`：本端发出/收到的字节速率。**两端对照**即可定位浪费在哪一段；
   - `收段(重复 x% 乱序 y%)`：收到段里"对端重传"和"乱序到达"的比例。重复 ≈ 发送端
     `重传` 说明重传的包真的过了链路（链路有余量，是窗口太大）；重复≈0 而重传高，说明
     重传的包没到对端（链路真丢包，或发送端本地出口队列溢出）。
     注意 `乱序` 里也含"原始包丢了、重传后才到"的段，**有损链路上 `乱序 ≈ 重传` 属正常**，
     不能据此判断多连接乱序；
   - `重传 ...(RTO a 快 b 提前 c)`：重传来源。`RTO` 占大头 = 包其实到了、只是超时判早了
     （在途 > BDP，该调小窗口）；`快`/`提前` 占大头且 `乱序` 高 = 乱序被当成丢包（见第 8 条）；
   - `重传`/`放大` 的括号外是**区间值（这 3 秒）**，括号内是累计值——判断"现在还在不在丢"
     只看区间值。
