...
func (p *SocksCli) GetPeer() (peer *Peer) {
	tsReuse := time.Now().Unix() - 60*3 // 3min 内的连接可以直接使用
	for {
		select {
	...
	for peer.TsCreate > tsReuse && peer.Peer.IsClosed() {
		peer.Close(context.TODO())
		peer = <-p.ChPeer
	}
...
func (p *SocksCli) close(ctx context.Context) (err error) {
	// 非阻塞 drain：channel 不会被 close，range 会永远阻塞
	drain := func(ch chan *Peer) {
		for {
			...
			case peer := <-ch:
				...
				if err1 := peer.Close(ctx); err1 != nil {
					err = err1
				}
			default:
				return
...
func (p *SocksCli) RunTCP(ctx context.Context, addr, toAddr string) (err error) {
	...
	for {
		select {
		case <-ctx.Done():
			log.Ctx(ctx).Info().Caller().Str("addr", addr).Msg("Done")
			return
		default:
		}
		c, err := l.Accept()
		...
		go p.connectTCP(ctx, toAddr, c)
	}
}

func (p *SocksCli) connectTCP(ctx context.Context, addr string, rc net.Conn) (err error) {
	return p.connect(ctx, addr, rc)
}
