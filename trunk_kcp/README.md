...
| --- | --- | --- | --- |
| 丢包主要来自"在途 ≫ BDP 时队列溢出"：`放大` 随窗口增大而升高、`瓶颈丢包` 高、吞吐在某个窗口达到峰值后**下降** | **限速**（整形器/QoS 限速） | 按 BDP 设（下面的公式），宁小勿大 | `nc=1` + `resend` 大（只等 RTO 也够） |
| 丢包与窗口无关：`重传 ≈ 路径丢包率` 基本不随窗口变、吞吐随窗口**单调上升**、`收段重复`≈0（重传的原始包真丢了） | **有损**（链路本身丢包） | **越大越好**（在途要撑住丢包造成的空洞），BDP 公式不适用 | `resend=2`（快速重传，别等 RTO）+ `nodelay=1`（minRTO 30ms） |
...
即：有损链路上吞吐 ≈ `在途 × (1-丢包率) / 修复时延`，所以窗口**翻倍就翻倍**，而修复时延
要靠 `resend=2`（重复 ACK 立刻重传，不用等 RTO）与 `nodelay=1`（minRTO 30ms）压下来。
`放大` 停在 `1/(1-丢包率)` 是**下限**，不是窗口没调好——别再为此缩窗口。
...
6. **真机有损链路推荐配置**：`kcp_sndwnd: 192`（越大越好，别用 BDP 公式）、
   `kcp_resend: 2`（重复 ACK 立刻重传，别等 RTO）、`kcp_nodelay: 1`（minRTO 30ms）、
   `kcp_nc: 1`、`kcp_rcvwnd: 1024`。2026-09-23 实测：RTT 54ms、2 连接、有损链路，
   **snd 32→192 使下载 60kB/s → 230kB/s（3.8x）**。
