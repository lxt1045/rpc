...
func NewCodec(ctx context.Context, rwc io.ReadWriteCloser, callers []Method, ctxPassKeys []string, needHeartbeat bool) (c *Codec, err error) {
	...
	c = &Codec{
		...
		delay:       delay.New[post](64, int64(time.Second)*60, true), // 增加到60秒以适应KCP重传
...
func (c *Codec) ReadLoop() {
	...
	for {
		...
		var header Header
		var bsBody []byte
		// During an upgrade the reader must stay alive until it consumes the
		// upgrade response. A status of 1 is the in-flight state, not a closed
