...
### `ReplySrc`（出包源地址）：云主机上**不要**填公网 IP

默认（零值）出包源 IP 取"触发报文的目的 IP"——服务端就是 SYN 到达的那个本地地址
（网卡内网地址）。云主机（腾讯云 VPC/轻量、阿里云 ECS 等）的公网 IP 由平台 1:1 NAT
提供，网卡只有内网地址，且平台对虚拟网卡做出站源地址校验：**源 IP 不是网卡地址的
报文会被静默丢弃**。所以这类环境必须留空——回包源地址是内网地址，由平台 NAT 转成
公网，与内核 TCP 完全同路。

`ReplySrc` 只适用于"平台不认识我们的 raw socket 流"的**无状态 DNAT/端口映射**环境
（Docker 桥接、K8s NodePort）：那时把源 IP 写成容器外的对外服务地址，等价于本端自己
做了 SNAT。判据：

```bash
# 服务端 tcpdump：SYN+ACK 的源地址
sudo tcpdump -vni any 'tcp port 18099'
#   源地址=公网 IP（网卡是内网）→ 配置了 ReplySrc，会被云平台丢 → 留空
#   源地址=网卡内网地址而客户端仍收不到 → 无状态 DNAT 场景 → 才需要 ReplySrc
```

`Listen` 启动时会检查 `ReplySrc` 是否为本机网卡地址：不是则打 WARN（列出上面两种
判读）；`Config.DebugPackets` 的 SYN 日志也会打印实际使用的 `回包源=<ip>`。
